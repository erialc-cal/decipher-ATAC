...
def make_data_loader_from_tscp(rnadata, atacdata, batch_size=64, context_discrete_keys=None):
    genes = torch.FloatTensor(rnadata.X.todense())
    atac = torch.FloatTensor(atacdata.X.todense())
    params = [genes, atac]
    context_tensors = []
